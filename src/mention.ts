...
import type { EmitterSource, Module, Range } from "quill";
...
export function createMention(
  quill: Quill,
  QuillStatic: typeof Quill,
  ModuleStatic: typeof Module
) {
  class Mention extends ModuleStatic<MentionOption> {
    ...
    insertItem(
      ...
      if (options.spaceAfterInsert) {
        this.quill.insertText(insertAtPos + 1, " ", QuillStatic.sources.USER);
        // setSelection here sets cursor position
        this.quill.setSelection(insertAtPos + 2, QuillStatic.sources.USER);
      } else {
        this.quill.setSelection(insertAtPos + 1, QuillStatic.sources.USER);
      }
      this.hideMentionList();
      return delta;
    }
